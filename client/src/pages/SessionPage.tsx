 ...
 * Mode mains libres avec défilement automatique + signal sonore + lecture vocale TTS
 */
...
export default function SessionPage() {
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center p-4">
        ...
        >
          ...
          <h1 className="font-display text-4xl font-bold text-foreground mb-4">Bravo !</h1>
          <p className="text-muted-foreground text-lg mb-2">{session.title} terminée</p>
          <p className="text-muted-foreground text-sm mb-8">Durée totale : {formatTime(totalElapsed)}</p>
          <p className="text-foreground/70 italic text-base mb-10 px-4">"{session.finishMessage}"</p>
          <div className="flex gap-4 justify-center">
            <Button variant="outline" size="lg" onClick={() => setLocation("/")} className="font-display">
              <ChevronLeft className="w-4 h-4 mr-2" />
              Accueil
            </Button>
            <Button size="lg" onClick={reset} className={`font-display bg-gradient-to-r ${colors.gradient} text-background hover:opacity-90`}>
    ...
    return (
      <div className="min-h-screen bg-background relative overflow-hidden">
        ...
        <div className="relative z-10 min-h-screen flex flex-col items-center justify-center p-4">
          <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.6 }} className="text-center max-w-lg">
            <div className="flex items-center justify-center gap-2 mb-6">
              <Heart className={`w-5 h-5 ${colors.text}`} />
              <span className={`font-display text-xs uppercase tracking-[0.25em] ${colors.text}`}>{session.subtitle}</span>
            </div>
            <h1 className="font-display text-4xl md:text-5xl font-bold text-foreground mb-4 leading-tight">
              {session.title}<br />
              <span className={colors.text}>{session.duration}</span>
            </h1>
            <p className="text-muted-foreground text-base mb-2">Niveau : {session.level}</p>
            <p className="text-muted-foreground text-sm mb-8 leading-relaxed max-w-md mx-auto">{session.description}</p>
            ...
            <div className="grid grid-cols-3 gap-4 mb-10">
              <div className="bg-card/60 backdrop-blur-sm rounded-xl p-4 border border-border/30">
                ...
                <span className="text-[10px] uppercase tracking-[0.15em] text-green-400 block">{session.phaseLabels.warmup}</span>
              </div>
              <div className="bg-card/60 backdrop-blur-sm rounded-xl p-4 border border-border/30">
                <div className="w-3 h-3 rounded-full bg-red-400 mx-auto mb-2" />
                <span className="text-[10px] uppercase tracking-[0.15em] text-red-400 block">{session.phaseLabels.workout}</span>
              </div>
              <div className="bg-card/60 backdrop-blur-sm rounded-xl p-4 border border-border/30">
                <div className="w-3 h-3 rounded-full bg-blue-400 mx-auto mb-2" />
                <span className="text-[10px] uppercase tracking-[0.15em] text-blue-400 block">{session.phaseLabels.cooldown}</span>
              </div>
            </div>

            <div className="flex items-center justify-center gap-4 mb-4 text-muted-foreground text-xs">
            ...
            <div className="flex flex-col items-center gap-2 mb-6">
              <div className="flex items-center gap-2 text-muted-foreground">
                ...
                <span className="text-xs">Signal sonore activé</span>
              </div>
              {speechSupported && (
                <div className="flex items-center gap-2 text-muted-foreground">
                  <Mic className="w-4 h-4" />
                  <span className="text-xs">Lecture vocale des instructions</span>
                </div>
              )}
            </div>

            {/* Voice toggle before start */}
            {speechSupported && (
              <button
                onClick={toggleVoice}
                className={`flex items-center gap-2 mx-auto mb-6 px-4 py-2 rounded-full text-xs font-display transition-all ${
                  voiceEnabled
              ...
              >
                {voiceEnabled ? (
                  <><Mic className="w-3.5 h-3.5" /> Voix ON — Instructions lues à voix haute</>
                ) : (
                  <><MicOff className="w-3.5 h-3.5" /> Voix OFF — Appuyez pour activer</>
                )}
              </button>
            )}

            <Button size="lg" onClick={startSession} className={`font-display text-lg px-10 py-6 bg-gradient-to-r ${colors.gradient} text-white hover:opacity-90 rounded-2xl`}>
  ...
  if (sessionState === "countdown") {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <motion.div key={countdownValue} initial={{ scale: 2, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.5, opacity: 0 }} transition={{ duration: 0.5, ease: "easeOut" }} className="text-center">
          <span className={`font-display text-[120px] font-bold ${colors.text} leading-none`}>{countdownValue}</span>
          <p className="text-muted-foreground text-lg mt-4 font-display tracking-wider">Préparez-vous...</p>
        </motion.div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <header className="sticky top-0 z-50 bg-background/90 backdrop-blur-md border-b border-border/30">
        <div className="container py-3">
          <div className="flex items-center justify-between">
            <button onClick={() => { cancelSpeech(); pause(); setLocation("/"); }} className="text-muted-foreground hover:text-foreground transition-colors flex items-center gap-1 text-sm">
              <ChevronLeft className="w-4 h-4" />
              <span className="hidden sm:inline">Quitter</span>
            </button>
            <div className="flex items-center gap-2">
              <Timer className="w-3.5 h-3.5 text-muted-foreground" />
              <span className="font-display text-sm text-muted-foreground tracking-wider">{formatTime(totalElapsed)} / {formatTime(session.totalDuration)}</span>
            </div>
            <div className="flex items-center gap-2">
              ...
              {speechSupported && (
                <button
                  onClick={toggleVoice}
                  className={`relative flex items-center justify-center w-8 h-8 rounded-full transition-all ${voiceEnabled ? "bg-green-500/15 text-green-400" : "bg-white/5 text-muted-foreground"}`}
          ...
          <div className="mt-3">
            <ProgressBar currentIndex={currentIndex} timeLeft={timeLeft} exerciseList={exerciseList} totalDuration={session.totalDuration} phaseLabels={session.phaseLabels} />
          </div>
        </div>
      </header>
      ...
      <main className="flex-1 container py-6">
        <div className="flex flex-col lg:flex-row gap-6 lg:gap-8">
          ...
          <div className="flex-1 flex flex-col items-center gap-6">
            <CircularTimer timeLeft={timeLeft} totalDuration={currentExercise?.duration ?? 45} phase={currentExercise?.phase ?? "warmup"} isTransition={currentExercise?.isTransition ?? false} />

            {/* Controls */}
            <div className="flex items-center gap-3">
              <Button variant="outline" size="icon" onClick={skipToPrev} disabled={currentIndex === 0} className="rounded-full w-10 h-10 border-border/50">
                <SkipBack className="w-4 h-4" />
              </Button>

              {sessionState === "paused" ? (
                <Button size="icon" onClick={resume} className={`rounded-full w-14 h-14 bg-gradient-to-r ${colors.gradient} hover:opacity-90`}>
                  <Play className="w-6 h-6 text-white" />
                </Button>
              ) : (
                <Button size="icon" onClick={pause} className="rounded-full w-14 h-14 bg-white/10 hover:bg-white/15 border border-white/10">
                  <Pause className="w-6 h-6" />
                </Button>
              )}

              <Button variant="outline" size="icon" onClick={skipToNext} disabled={currentIndex >= exerciseList.length - 1} className="rounded-full w-10 h-10 border-border/50">
            ...
            {speechSupported && voiceEnabled && (
              <div className="flex items-center gap-2 text-xs">
                <div className={`w-2 h-2 rounded-full ${isSpeaking ? "bg-green-400 animate-pulse" : "bg-green-400/30"}`} />
                <span className="text-muted-foreground">{isSpeaking ? "Lecture en cours..." : "Voix activée"}</span>
              </div>
            )}
            ...
            {currentIndex < exerciseList.length - 1 && (
              <div className="w-full max-w-xl">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground">Prochain</span>
                </div>
          ...
          <aside className="hidden lg:block w-72 shrink-0">
            <div className="sticky top-40">
              <h3 className="font-display text-xs uppercase tracking-[0.2em] text-muted-foreground mb-4">Programme ({totalMainExercises} exercices)</h3>
      ...
      <AnimatePresence>
        {sessionState === "paused" && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-40 bg-background/80 backdrop-blur-sm flex items-center justify-center" onClick={resume}>
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="text-center" onClick={(e) => e.stopPropagation()}>
              <Pause className="w-16 h-16 text-foreground/30 mx-auto mb-4" />
              <h2 className="font-display text-3xl font-bold text-foreground mb-2">En pause</h2>
              <p className="text-muted-foreground mb-8">Appuyez pour reprendre</p>
              <div className="flex gap-4 justify-center">
                <Button variant="outline" size="lg" onClick={reset} className="font-display">
                  <RotateCcw className="w-4 h-4 mr-2" />
                  Recommencer
                </Button>
                <Button size="lg" onClick={resume} className={`font-display bg-gradient-to-r ${colors.gradient} text-white hover:opacity-90`}>
